    ...
    try {
      ...
      if (lowerInput.includes("what meals can i make")) {
        ...
        if (fridgeItems.length === 0) {
          ...
          const formattedListForGPT = fridgeItems.join(', ');
          const markdownListForDisplay = fridgeItems.map(item => `• **${item}**`).join("\n");
          responseText = `I see you have:\n${markdownListForDisplay}\n\nHere are some meal ideas:\n`;
          const prompt = `I have these ingredients in my fridge: ${formattedListForGPT}. Please suggest 3 creative and diverse meal recipes using only these ingredients. For each recipe, do the following:
